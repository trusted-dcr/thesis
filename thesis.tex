...
% Description of components covered in this section and why they are needed.
	...
	The following is a security analysis of the algorithm described in section \ref{sec:analysis}.
		...
		As such, an adversary can at most create attacks by crashing processes, message omissions, message delays and tampering with messages, where tampering will be noticed by the correct peers, as the adversary does not have the MAC secret.
		...
		One such chaining look like this: omit messages from the leader to $f+1$ processes until one of them, $p_1$, becomes a candidate.
		...
		And due to election pre-election phase, the original leader will still be the leader after the attack has been terminated.
		...
		Basically, the attack is for the adversary to omit all client messages to the leader of cluster.
		...
		Notice that both these attacks happens on a single cluster, and therefore at most affect a subset of the clusters that make up the entire system.
		...
		An alternative is to attack the clusters of all dynamically independent event for the current $G$ state, but the adversary is not guaranteed that the global state reported by the log-augmented snapshot described in~\ref{subsec:global-history-collection}, and is therefore guaranteed that their attack will completely prevent any further execution of graph.
