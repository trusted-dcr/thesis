...
\section{Introduction}
	...
	In an increasingly digital society where distributed, decentralized solutions prosper, the natural extension of such a development is the decentralization of arbitrary smart contracts.
	One such extension is \textit{Ethereum}, which achieves it using blockchain technologies.
	An unfortunate side-effect of Ethereum is the currency, \textit{Ether}, used to pay for execution of code, which is subject to the blockchain bubble, making executions of smart contracts inordinately expensive.
	Furthermore blockchains are based on the premise that all nodes have access to all of the data in the blockchain, which leads to considerations regarding scalability.
...
\section{Intel SGX}
	...
	If \texttt{D} is also executed after \textt{A} has been executed, then it is unclear how a combined history of executions would look like, as there are multiple valid points in the sequence for the execution of \texttt{C}.
