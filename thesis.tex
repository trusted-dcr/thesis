...
\section{Background}
	...
	A DCR graph is a representation of a workflow, made up of one or more events with a number of relations between them. 
		...
		\begin{itemize}
			...
			\item We can now use a consensus subprocess to decide on a $v$, which will then be executed.
		...
		In order to circumvent this impossibility, the guarantees of a fault-tolerant system that utilises a consensus protocol are relaxed, for instance by guaranteeing Termination only when no faults are present, as is the case in  the Paxos protocol~\cite{lamport_part-time_1998}. 
		...
		This eases some of the inherent problems in consensus, as seen in~\cite{liu_scalable_2016} and~\cite{dhillon_hyperledger_2017}.
			...
			A transition has the form $(s_{from}, i, o, s_{to})$, where $i$ is an input consisting of tuple of a bit-string and the channel the bit-string was received on, and $o$ is the output consisting of a set of bit-string/channel pair, each representing that bit-string being sent on the appertaining channel.
			...
			This potentially includes new state machines with new current states and new channels.
			...
			\begin{enumerate}
				\item Add all the state machines to the integrity protected area of the process.
					...
					\end{itemize}
				\item For each endpoint state machine in unreliable channels between processes, add a new state machine in the unprotected area of each of the processes, connect a new unreliable channel between these and add a reliable channel from the old endpoint state machines to the new.
					\begin{itemize}
			...
			\begin{figure}[ht!]	
				\center
				\includegraphics[scale=0.45]{figures/state-machines/NFA-transformation.pdf}
				\caption{Transformation of figure \ref{fig:simple-nfa} from state machine in crash-resistant protocol to integrity protected state machine in byzantine fault-tolerant protocol, given that all channels are to other processes.\label{fig:nfa-transformation}}
			\end{figure}

			These transformations gives us the following guarantees, if a process experiences a byzantine fault:
			\begin{itemize}
				\item If the state machines in the integrity protected area are replaced with similar state machines, $SM_s$ will identify them as being incorrect and refuse to MAC their messages, which will result either (1) the message will be not be MAC or MACed with the wrong key by the sending state machine, (2) the message being dropped by the faulty state machine or (3) one of the state machines crashing.
				...
				So the same three cases will still hold.
	...
	This optimization is, however, only possible when, in example graph 3, the state of \texttt{C} is known and since locking is only performed forwards, there is no implicit way to propagate executions backwards.
