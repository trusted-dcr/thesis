...
\section{Introduction}
	...
	A state change is only required to be propagated to the nodes which that state change matters to, with respect to \textit{g}.
...
\section{Background}
	...
	A DCR graph is a representation of a workflow, made up of one or more events with a number of relations between them. 
		...
		This eases some of the inherent problems in consensus, as seen in~\cite{liu_scalable_2016} and~\cite{dhillon_hyperledger_2017}.
			...
			A finite state machine consists of a current state, $s_{current}$, a set of a finite set of $n$ distinct states, $S=\{s_1, s_2, \dots, s_n\}$, and a finite set of $m$ distinct state-transitions, $T$.
			A finite state machine can only be in one state at any time, and transitions to another state by accepting any bit-string as input, and optionally outputs another bit-string. 
			A transition has the form $(s_{from}, i, o, s_{to})$, where $i$ is an input consisting of tuple of a bit-string and the channel the bit-string was received on, and $o$ is the output consisting of a tuple of a bit-string and the set of channels that bit-string will be sent on.
			...
			The state machine inside the process can be connected with reliable channels. 
			...
			Each state machine is connected to the other on the same process with reliable channels $c_0-c_3$, and $SM_1$ on $p_0$ is connected to $SM_3$ on $p_1$ with the unreliable channel $c_4$.
			...
			We assume that any byzantine fault cannot replicate $SM_{s}$, which models that faulty processes cannot access the cryptographic secrets of a correct process.
			This is a reasonable assumption by the integrity and confidentiality guarantees given by Intel SGX, if the secrets are stored in an enclave (see section \ref{subsec:enclave}).
			...
			\begin{enumerate}
				\item Add all the state machines to the integrity protected area of the process.
					\begin{itemize}
						...
						Please note that it is rarely necessary to make all components into enclaves, only the security-%something-something TODO MANGLER HUSK SDF
					\end{itemize}
				...
				\item Add a Remote Attestation state machines to each process and a Remote Attestation process.
					\begin{itemize}
			...
			\begin{itemize}
				...
				This is handled by the $SM_s$ and $SM_A$, which will contact $P_RA$ and be Attested and securely provisioned. 
				In our example, we have only added a single $P_RA$, which of course means that a \textit{k}-resilient protocol suddenly cannot handle a single $P_RA$ crash. 
				This can be solved by adding \textit{k} identical $P_RA$s.
				...
				\item Whenever a message that supposedly originates from another process is received from the unprotected area  by a state machine in the integrity protected area, the MAC of that message must be verified by $SM_s$.
					\begin{itemize}
						\item In case this verification fails, the message must be dropped discarded.
					\end{itemize}
			...
			\begin{figure}[h!]	
				...
				\caption{Transformation of figure \ref{fig:simple-nfa} from state machine in crash-resistant protocol to integrity protected state machine in byzantine fault-tolerant protocol.\label{fig:tranformation-model}}
			\end{figure}

			These transformations gives us the following guarantees, if a process experiences a byzantine fault:
			\begin{itemize}
				\item If the state machines in the integrity protected area are replaced with similar state machines, $SM_s$ will identify them as being incorrect and refuse to MAC their messages, which will result either (1) the message will be unMACed or MACed with the wrong key, (2) the message being dropped by the faulty state machine or (3) one of the state machines crashing.
