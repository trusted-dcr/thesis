...
\section{Introduction}
	...
	A core necessity of DCR is the agreement of all parties on what has transpired in the workflow and what the state of the workflow is at any given point, a non-trivial problem in decentralized DCR.
	DCR graphs allow for highly localized states, as nodes in a DCR graph are only dependent on, and can only effect, nodes within a limited range, graph-wise.
	Depending on how a specific DCR graph is constructed, it can therefore allow for a very high degree of concurrency.
	As DCR graphs are concerned with maintaining a consistent state, distributed DCR graphs lead to the well-known issue of state replication.
	...
	As DCR graphs have innate properties which would be very limited by existing solutions to the state replication problem, the lesser known partial state replication seems opportune.
	Partial state replication is a significantly less understood problem, but is increasing in importance, as scalability issues of popular decentralized databases, such as Bitcoin's blockchain, present themselves.
	The tools afforded to us by recent technological advances, provide strong guarantees and advantages which have already made waves within state replication solutions and could potentially be leveraged to improve existing solutions to the problem of partial state replication.
	...
	The goal is therefore a decentralized DCR engine distributed among a number of actors, which allows for a graph to be distributed to and interacted with by each actor, in accordance with the specifications of the graph.
	...
	At any given time, an actor should be able to collect the state of the graph and at be ensured that any future state will not be incompatible with a previously collected state, with regards to the rules specified by the graph.
	...
	An existing platform which partially provides a solution to the goal of this project is Ethereum~\cite{_wiki_2018}.
...
\section{Background}
	...
	A DCR graph is a representation of a workflow, made up of one or more events with a number of relations between them. 
			...
			The notion of concurrency in DCR graphs, described in\cite{debois_concurrency_2015}, is the property a pair of events is said to have when the graph allows those events to be executed in either order with the same result.
			More formally events are concurrent, when the following principles are applicable:
			\begin{itemize}
				\item Adjacent concurrent activities should be able to be executed in either order\footnote{Adjacent meaning in a valid execution sequence.}.
				\item Concurrent activities cannot enable each other.
				\item Concurrent activities cannot disable each other.
				\item Concurrent activities cannot have incompatible effects on the state of other activities.
			\end{itemize}

			Given that concurrency does not regard the state of the graph, when evaluating the validity of the principles mentioned above, there is the need for further specification.
			For this, the term event independence is used.
			...
			\begin{description}
				...
				\item[Static independence] means that for two events, are they independent in every state they could be in. This problem belongs to NP and an approximation will have to be used instead.
			\end{description}
		...
		When we take the proofs of sufficient and necessary amounts of processes in BFT systems into account, these advanced strongly indicates that TEEs might be able to reduce byzantine faults to crash faults, a statement we will explore further in section \ref{subsubsec:transforming-byzantine-faults}.
		...
		This eases some of the inherent problems in consensus, as seen in~\cite{liu_scalable_2016} and~\cite{dhillon_hyperledger_2017}.
			...
			\subsubsection{Transforming byzantine faults to crashes}
			\label{subsubsec:transforming-byzantine-faults}
			We conjecture that the correct use of Intel SGX can transform all byzantine faults in a distributed system with unreliable channels to crashes and lost messages.
			...
			To model this, we introduce the notion of a cryptographic secret state machine $SM_{s}$, and an integrity protected area of the process.
			...
			The transformation from a crash-resistant protocol to a byzantine-fault-tolerant protocol consist of 4 steps:
			\begin{enumerate}
				...
				\item Add a Remote Attestation state machines, $SM_A$, to each process and a Remote Attestation process, $P_{RA}$ to the protocol.
					\begin{itemize}
						\item We will not go into detail with the Remote Attestation protocol here.
						We will simply use it to securely provision $SM_s$ with a symmetric key.
						For more information on the Remote Attestation protocol, see section \ref{subsec:attestation}, \textit{Intel SGX explained}~\cite{costan_intel_2016} and/or \textit{Intel SGX Developter guide}~\cite{intel_sgx_guide}.
					\end{itemize}
			\end{enumerate}
			...
			After this transformation, some steps must added to the internals of protocol:
			\begin{itemize}
				\item Before running the protocol, a pre-compute step must be added where each process' $SM_s$ is provisioned with the same cryptographic secret. 
				...
				This means that the $P_{RA}$s plays no further role after this step in the protocol.
				...
				\item In the event that $SM_s$ is asked to sign a message from a state machine that it identifies is not the original state machines in the integrity protected area, it discards the message.
			\end{itemize}

			Notice that this transformation is polynomial in the number of messages, under the assumption that the MAC, and MAC-verification sub-routines in $SM_s$ are polynomial operations in the length of the message.
			...
			\begin{itemize}
				\item If the state machines in the integrity protected area are replaced with similar state machines, $SM_s$ will identify them as being incorrect and refuse to MAC any messages, which will result either (1) the message will be not be MAC or MACed with the wrong key by the sending state machine, (2) the message being dropped by the faulty state machine or (3) one of the state machines crashing.
				In case (1), $SM_s$ on any correct process who receives the message will recognise that the MAC is invalid, and the correct integrity protected state-machines will drop the message and revert to the previous state.
				This is equivalent to the message never being sent, which by definition the protocol can handle, as it can handle unreliable channels between processes.
				In case (2), the message will be dropped by the faulty process, which is again is equivalent to the message being dropped on an unreliable channel.
				Case (3) is a crash, which the crash-resistant protocol can already handle.
				\item If $SM_s$ itself is replaced, it can no longer MAC messages, as it no longer has the appropriate key, and cannot be provisioned with one.
				This results in the same three cases as above.
				\item If one or more of the state machines in the integrity protected area are removed, it is equivalent to a crash. Any correct state machine sending messages to the removed state machine, and will not get a reply, which is the same behaviour as a crash.
				\item If any state machines are added to the integrity protected area, they are inconsequential, as the other state machines in the area cannot communicate with them without being replaced.
				\item If the state machines outside the integrity protected area are replaced, it can result in either (1) replacing the messages to/from the integrity protected area, (2) dropping messages to/from the integrity protected area or (3) inducing crashes in other state machines, as well as itself.
				Case (1) is equivalent to dropping the messages, by much the same argument as above: $SM_s$ will recognise that the MAC is invalid, and the state machines in the correct processes will ignore the message.
				Case (2) is a message loss, and case (3) is a crash, both of which the crash-resistant protocol can already tolerate.
			\end{itemize}

			This concludes the argument that Intel SGX and cryptography can change byzantine faults to crash faults, and thereby, using the above transformation, make a crash-resistant protocol into a byzantine-fault-tolerant protocol.

			\subsubsection{Transforming a real-life protocol}
			We will know show a practical example of how the transformation in section \ref{subsubsec:transforming-byzantine-faults} can be utilised.
			...
			We will now show how the transformation from section \ref{subsubsec:transforming-byzantine-faults}, can transform CSME to give the same guarantees in a byzantine system as the untransformed protocol gives in a crash-fault system.
			...
			This is done by rules for state machine transformation in section \ref{subsubsec:transforming-byzantine-faults}. 
			...
			Having transformed the CSME protocol, we will now show that the transformed protocol will exhibit the same behaviour under byzantine faults, as the untransformed protocol does under crash faults.
			...
			The following is a systematic walkthrough of any possible byzantine faults on the processes, and their results:
			\begin{itemize}
				...
				\item If a process gets an additional state machine in the integrity protected area
				\item If a process gets an additional state machine outside the integrity protected area
				...
				\item If a process has a corrupted wrapper state machine % UPS!
			\end{itemize}
			...
			Thereby, any client process (figure \ref{fig:CSME-client-NFA-transformed}) will get stuck in a loop between $s_1$ and $s_1'$, when $SM_s$ rejects the (missing) MAC of the token. 
			...
			% forklar algorithm - ME1, ME2 og ME3 *
			% ikke decideret crash-resistant - bevarer safety under crashes *
			% intet er sikkert under byzantinske fejl *
			% lav state machines (måske n SMs på server process? måske critical section SM på clients? Men hvordan sikrer vi den mod byz?)* 
			% lav tranformation *
			% lav byzantisk fejl på server
			% Vis at det reducerer til crashes og tabte beskeder
			% hvad med at sende tokens til forkerte clients? Ligger det sig bare under eventual liveness?
			% Hvad med to byzantiske fejl, sådan at server ikke signer, og client ikke verificerer?
				% Der er nok et argument for at de ikke kan påvirke KORREKTE processor.
	...
	As discussed in section \ref{sec:algorithm}, the execution of non-independent events must be synchronised.
	...
	As DCR only allows the execution of an activity to affect events in the neighbourhood, (3), and the second neighbourhood, (4), of the executing event, locking need only be performed within this set.
