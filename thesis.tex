...
% Description of components covered in this section and why they are needed.
	...
	\textit{2} & $\lfloor \frac{(n-1)}{2} \rfloor$          & $n$                      & $n-1$                       & $n^2$                     & $2\cdot\lfloor\frac{(n-1)}{2}\rfloor =O(n)$      & $\leq$ Dynamic independence & $L$ \\ \hline
	\textit{3} & $\lfloor \frac{(m-1)}{2} \rfloor$ (partly) & $n$                      & $m-1$                       & $n^2$                     & $2\cdot\lfloor\frac{(m-1)}{2}\rfloor =O(m)$      & Dynamic independence        & $L$ \\ \hline
	\textit{4} & $\lfloor \frac{(m-1)}{2} \rfloor$ (partly) & $n$                      & $m-1$                       & $m^2$                     & $2\cdot\lfloor\frac{(m-1)}{2}\rfloor =O(m)$      & Dynamic independence        & $L\cdot \frac{n}{m}$ \\ \hline
	...
	After an execution of an event $e$, each of the peers replicating $e$ and $e$'s dynamic dependency set is informed by $L$ of that execution, with a monotonic counter value that enables ordering of the events' execution.
	...
	This ensures that if a leader dies, a new leader can be chosen (using $O(n^2)$ messages), which gives us the aforementioned fault tolerance.
	...
	The system still utilizes a single leader, which synchronises all requests.
	When event $e$ is requested executed, the leader runs an SMR algorithm similar to the one described in approach \textit{2}, but only with cluster for $e$, and the clusters of $E_{eff}(G,e)$ -- to ensure that the clusters of $E_{eff}(G,e)$ has updated the state/enabledness of the events they are responsible for.
	This ensures a lesser message complexity than approach \textit{2}, given $n > m * (|E_{eff}(G,e)|+1)$.
	Notice that $n = m \cdot v \geq m * (|E_{eff}(G,e)|+1)$).
	$L$ must still synchronise all requests, and worse, cannot allow for concurrent request commits, since the peers would not necessarily be able to recreate the synchronisation between these.
	...
	This has the benefit that even if a leader dies, the recovery has message complexity $m^2$ (instead of $O(n^2)$), and the number of concurrent client requests are no longer bounded by the speed of a single process.
	...
	The fault tolerance of this approach is quite bad, as any peer crashing will result in a data loss, and effect availability, even though it has the same property as approach \textit{3} and \textit{4} of not effecting the events in $I_stat\{e\}$
	...
	\subsection{Event Locking} % Missing a conclusion: When do we lock what? We have only described cases but not what our locking algorithm is.
	...
	PBFT               & $\lfloor \frac{n-1}{3} \rfloor = f$ & $2(2f+1)$               & $(2f+1)+(2f+1)^2$            & Byzantine 				     \\ \hline
	...
	Chandra-Toueg      & $\lfloor \frac{n-1}{2} \rfloor = f$ &                         & $4(f+1)+(f+1)^2$             & Crash                         \\ \hline
	Multi-Paxos        & $\lfloor \frac{n-1}{2} \rfloor = f$ &                         & $\sim3(f+1)$ to $\sim5(f+1)$ & Crash                         \\ \hline
