...
% Description of components covered in this section and why they are needed.
	...
	\begin{table}[!ht]
	\centering
	\makebox[\textwidth][c]{\begin{tabular}{ p{0.2cm} p{1.5cm} p{1.5cm} p{1.5cm} p{1.5cm} p{2cm} p{2.3cm} p{1.5cm} }
	\toprule
	\#         & Fault tolerance (Availability)             & Fault tolerance (Safety) & Fault tolerance (Data loss) & Crash recovery (messages) & Message complexity for executing $e$ (Best case) & Concurrency                 & Request Bottleneck    \\ 
    \midrule
	\textit{1} & $0$                                        & $n$                      & $0$                         & n/a                       & $2$                                       		& $\leq$ Dynamic independence & $L$ \\
	\textit{2} & $\lfloor \frac{(n-1)}{2} \rfloor$          & $n$                      & $n-1$                       & $O(n)$                    & $2\cdot\lfloor\frac{(n-1)}{2}\rfloor$     		& $\leq$ Dynamic independence & $L$ \\
	\textit{3} & $\lfloor \frac{(m-1)}{2} \rfloor$ (partly) & $n$                      & $m-1$                       & $O(n^2)$                  & $2\cdot\lfloor\frac{(m-1)}{2}\rfloor$     		& Dynamic independence        & $L$ \\
	\textit{4} & $0$ (partly)                               & $n$                      & $0$                         & n/a                       & $2\cdot D_{dyn}(G,e)$           					& Dynamic independence        & $L\cdot n$ \\
	\textit{5} & $\lfloor \frac{(m-1)}{2} \rfloor$ (partly) & $n$                      & $m-1$                       & $O(m)$                    & $2\cdot\lfloor\frac{(m-1)}{2}\rfloor$     		& Dynamic independence        & $L\cdot \frac{n}{m}$ \\
	\bottomrule
	\end{tabular}}
	...
	Approach number \textit{4} is heavily inspired by~\cite{hildebrandt_safe_2011}.
	...
	The fault tolerance of this approach is quite bad, as any peer crashing will result in a data loss, and effect availability, even though it has the same property as approach \textit{3} and \textit{5} of not effecting the events in $I_{stat}\{e\}$

	\paragraph{Partial state, multi-leader approach}
	Approach number \textit{5} is our chosen approach, and we will go into detail with it below.
	...
	From here on in, we will primarily concern ourselves with the details of approach number \textit{5}:
