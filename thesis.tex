...
% Description of components covered in this section and why they are needed.
	...
	The problem of distributed DCR has several parts which must each be considered individually.
	Consider a solution where a DCR graph is distributed among peers, such that each peer is only responsible for one or more events.
	Responsible meaning that that peer is able to be involved in making state changes for that peer, on request.
	...
	The first issue then presents itself, in that simultaneous client requests can be incompatible and therefore needs to be applied in the same order for all affected peers.
	This algorithm solves that problem with performing locking before committing, leading to the first key problem of the algorithm:
	\begin{description}
	...
	\begin{description}
		\item[Consensus] In extension to preventing simultaneous incompatible executions, locking has pushed the issue of concurrency into the locking portion, which now needs to be handled in the sense that peers must agree who hold the locks.
		Furthermore avoiding deadlocks and ensuring progression is obviously a concern.
	\end{description}
