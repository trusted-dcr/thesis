...
s
	...
	\textit{3} & $\frac{(m-1)}{2}$ (partly)     & $n$                      & $m-1$                       & $2 \cdot \frac{(m-1)}{2}$                          & Total            & $L$                   \\ \hline
	\textit{4} & $\frac{(m-1)}{2}$ (partly)     & $n$                      & $m-1$                       & $2 \cdot \frac{(m-1)}{2}$                          & DCR concurrency & $L \cdot \frac{n}{m}$ \\ \hline
	\textit{5} & $0$ (partly)                   & $n$                      & $0$                         & $2 \cdot D_{dyn}(G,e)$ ($e$ executing)			  & DCR concurrency & $L$                   \\ \hline
	\end{tabular}}
	\caption{Comparison of different approaches.
	\textit{1}: Central Server.
	\textit{2}: State Machine Replication.
	\textit{3}: Event state distributed and replicated with full consensus.
	\textit{4}: Event state distributed and replicated with dependency consensus.
	\textit{5}: Event state distinctly distributed (2PC).}
