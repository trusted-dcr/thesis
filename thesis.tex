...
\section{Introduction}
	...
	The trivial method of ensuring fault tolerance, is to make all peers responsible for the entire state of the system, but given that changes in the graph would then have to be synchronized with all peers, this would not allow for achieving consensus in less than $O(n)$ messages.
	...
	\begin{description}
		\item[Concurrent] Allow for a high degree of concurrency as permitted by DCR graphs, as shown in~\cite{debois_concurrency_2015}.
        \item[Scalable] Achieve consensus on a state change in less than $O(n)$ messages, where $n$ is the number of peers in the graph, while supporting the distribution of the state of the DCR graph among those peers.
...
\end{description}
	...
	We denote this execution $E(G,e)=G'$.
			...
			\begin{description}
				...
				The \textit{included} attribute is denoted by the outline of the event: \ev{A} when included is true and \ev[000]{A} when included is false.
			...
			\begin{itemize}
				...
				\item $R = \{r_1, r_2, \dots r_e\}$ is the set of relations in the workflow, where each relation, $r$, has the form $r=(e_{from}, e_{to}, Type)$ and $e_{from}$ is the event $r$ is originating from, $e_{to}$ is the event $r$ is targeting, and $Type$ is one of the relation types described in Section~\ref{subsubsec:relations}.
			\end{itemize}
		...
		Consider a simple graph: $\ev{A} \crel \ev{B}$, and let a subset of peers, $ps_1$, be responsible for the state of $\ev{A}$, and a subset of peers, $ps_2$, be responsible for the state of $\ev{B}$.
		...
		These systems are knowns as \textit{byzantine fault tolerant} (BFT) systems.
		...
		In order to access any of these, an enclave therefore needs a so-called \textit{wrapper component} which is a non-enclave process that make calls to, and receive calls from enclave processes.
