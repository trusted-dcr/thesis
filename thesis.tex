...
\end{description}
	...
	A method of discerning which events need to be locked and when is described in Section~\ref{subsec:event-locking}.
	...
	% Definition of lockign: A lock on an event is defined as ... we say that event Alocks event B
	...
	This would require incorporation of the snapshot algorithm into the Raft algorithm, significantly bloating the log used by the algorithm.
...
\end{minipage}
	...
	\begin{enumerate}
		...
		This requires $O(n)$ messages, where $n$ is the number of event clusters.
		...
		For each of $m$ executions in each of $n$ returned sequences, the event which that execution pertains' returned sequence is checked for the presence of that execution.
		...
		For a graph with $n$ events and $m$ unique executions performed during the lifetime of the graph, this then has computational complexity of $O(n\cdot m)$.
		\item Creating a graph from execution relationships. As each relationship is represented with an edge, this has computational complexity of $O(e)$ where $e$ is the number of execution relationships.
		\item Finding a topological ordering. 
		Using the DFS traversal algorithm, means visiting each node twice via its edges and therefore this operation has a computational complexity of $O(|V|+|E|)$, where $V$ is the vertices in the graph and $E$ is the edges in the graph. %could be rewritten to something like $O(n \cdot m + |E|)$ or even reducing E as well?
	\end{enumerate}

	In conclusion the global history can be collected in $O(n)$ messages and within $O(n\cdot m)$ computational complexity.

	\subsection{Raft}
	\label{sub:raft}
	Raft~\cite{ongaro_search_2014} is a variation of multi-Paxos establishing consensus between processes over a continuous stream of commands.
