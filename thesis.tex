...
% Description of components covered in this section and why they are needed.
	...
	For a global history to be collectible, each event must track more than just their own executions.
	...
	There are, however, cases where locking is not performed, as locking is mostly concerned with preventing race conditions, but where enabledness is affected.
	...
	A consistent cut means that all process events obey the happened-before rules within the cut, meaning:
	\begin{itemize}
	...
	Given a cut from a DCR graph, it is always possible to reduce the cut until consistency is at some point attained.
	...
	A global history collection is started, by a client requesting it from every cluster, and \texttt{A} is the first event to send its execution sequence $\{\}$. After sending the empty execution sequence it executes.
	...
	Since all messages received within the cut are discarded along with all later executions if the message is not sent within the same cut, it follows that it is eventually possible to obtain a consistent cut, in the worst case an empty sequence of executions.

	In fact, this method guarantees a consistent cut containing the entire execution sequence of the process that recieved the request first (in the happened-before sense of first), due to the rules of happened-before.
	...
	Note that a reverse postorder DFS graph traversal is exactly a topological order, if the graph is a directed \textit{acyclic} graph~\cite[chapter 4]{sedgewick_algorithms_2011}.
	If the algorithm reaches a vertex that has already been marked, there are two possibilities of why it is marked:
	\begin{enumerate}
		\item The marked neighbour has been visited in an earlier DFS traversal, and thus will correctly appear after the current vertex in the ordering.
...
\end{minipage}
	...
	There are thereby four different steps to the process of collecting the execution history of a graph.
	\begin{enumerate}
	...
	The following is a security analysis of the algorithm described in section \ref{sec:analysis}.
		...
		We allow for a strong adversary known as a Dolev-Yao adversary~\cite{dolev_security_1983}, which is an adversary that can coordinate faulty nodes, delay communication and delay correct nodes, but is unable to subvert the cryptographic techniques described above, and does not have access to our cryptographic secrets.
		...
		This is a quite strong assumption, in that Intel does not necessarily share priorities with the peers and users of the system, and can, in some situations, have incentive to surrender these keys.

		\subsection{Analysis}
		Essentially, an adversary has the same limitations in attacking the system, as a byzantine fault has in breaking it.
		...
		Assuming that the adversary cannot continue omitting messages, however, the cluster will eventually successfully complete an election, and resume operation.
		...
		Note that this is an attack that exactly simulates the FLP-impossibility in Raft.
		FLP-impossibility~\cite{fischer_impossibility_1985} shows that there will always exist a sequence of message omissions that violates availability (i.e. liveness in SMR), if the system is an asynchronous setting and handles crashing processes.
		As such, any fault-tolerant SMR in an asynchronous setting will have such an attack.
		...
		The attack consists of the adversary to omitting all client messages to the leader of cluster.
