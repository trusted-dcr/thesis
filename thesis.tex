...
\section{Introduction}
	...
	\begin{itemize}
		...
		\item Achieve consensus on the state of a decentralized DCR graph in a number of messages less than the number of nodes in the graph.
...
% Description of components covered in this section and why they are needed.
	...
	Responsible meaning that that peer is able to be involved in making state changes for that event, on request, and must agree with the other responsible peers on the current state of that event.
	...
	The first issue then presents itself, in that simultaneous client requests can lead to execution of dynamically dependent events, and therefore needs to be applied in the same order for all affected peers.
