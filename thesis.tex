...
% Description of components covered in this section and why they are needed.
	...
	An aggregation of these sequences needs to be found, which can be accomplished through the construction of a directed graph and topologically ordering the vertices.
	Before an aggregation can be performed, we define each sequence as a set of execution relationships.
	...
	A topological ordering of event executions is obtained by performing a Depth-First Search (DFS) in a Reverse Post-Order (RPO).
	The DFS traversal is started at an arbitrary node and once it reaches a leaf node, it backtracks.
	For each node reached while backtracking, if that node only has child nodes which have already been visited is prepended to the result.
	If a node encountered while backtracking has child nodes which have not yet been visited, a recursive DFS-RPO traversal is started at that child node.
	...
	\begin{enumerate}
		...
		If an execution is deleted this way, it could have cascading effects, but under the assumptions that look-ups in the aforementioned table and checking whether or not a sequence contains a specific execution is constant.
		For a graph with $n$ events and $m$ unique executions performed during the lifetime of the graph, this has computational complexity of $O(n\cdot m)$.
		...
		\item Finding a topological ordering. Using DFS-RPO, means visiting each node twice via its edges and therefore this operation has a computational complexity of $O(v+e)$, where $v$ is the number of vertices in the graph and $e$ is the number of edges in the graph.
	\end{enumerate}
...
$\MSG{\RSP\ELECTION, term, success}$
	...
	\begin{mdframed}[backgroundcolor=Papyrus]
	\begin{lstlisting}[style=pseudo, caption={Logic shared between follwer, candidate and leader states.}]
$log \leftarrow []$
$term \leftarrow 0$
