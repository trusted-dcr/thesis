...
\section{Introduction}
	...
	\begin{itemize}
		...
		\item An algorithm for achieving consensus in a decentralized and distributed state replication system, using less than $O(n)$ messages.
...
% Description of components covered in this section and why they are needed.
	...
	For these reasons, we present a simple method of collecting the global state of a DCR graph in subsection \ref{subsec:global-state-collection}.
...
\end{lstlisting}
	...
	\label{subsec:global-state-collection}
	...
	Any sequence returned from an event, is a valid sequence and therefore compatible with any other sequence returned from an event.
	Each execution of an event is uniquely identifiable as part of the locking algorithm, as locks have to be distinguishable from one another.

	\begin{figure}[ht!] 
		\center
		\includegraphics[scale=0.6]{figures/dcr-graphs/GlobalStateCollectionExample.pdf}
		\caption{Example graph illustrating global state collection.}
		\label{fig:global-state-collection-example}
	\end{figure}

	From the graph shown in Figure \ref{fig:global-state-collection-example}, the shown sequences are collected for each event.
	As each of these sequences are valid, they can also be shown as a happened-before relationships REF, for example the execution sequence recorded by \texttt{B} could also be formulated as: $A_1 \rightarrow B_1, B_1 \rightarrow A_2, A_2 \rightarrow D_1, D_1 \rightarrow D_2$.
	These relationships can be trivially aggregated to compile the global state of the graph, with the exception of situations where both $A_1 \rightarrow B_1$ and $B_1 \rightarrow A_1$ are present, meaning that \texttt{A} and \texttt{B} are independent.
	This is in violation of traditional happened-before relationships as they have the properties of transitivity, irreflexivity and antisymmetry, whereas the legal execution sequences of DCR graphs have the properties of transitivity, reflexivity and symmetry, due to concurrent activities\footnote{It follows from if $A \rightarrow B$ and $B \rightarrow A$, then if transitivity holds then $A \rightarrow A$ and $B \rightarrow B$.}.
