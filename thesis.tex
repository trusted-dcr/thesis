...
% Description of components covered in this section and why they are needed.
	...
	From each execution set, or set of execution ordering rules, an aggregation can be computed by performing a pairwise comparison of sequences.
	We define a sequence as $S_n$ where $n$ is the node that returned that sequence.
	Each sequence consists of up to $m$ elements, where $m$ is the number of executions performed in the entire graph.
	...
	For the aggregation, $S_a$ of two sequences, $S_1$ and $S_2$, we compare each element in each sequence starting from the left and apply the following rules:
	\begin{itemize}
		\item if the compared elements are dissimilar, the element from $S_1$ is appended to $S_a$ and the subsequent element from $S_1$ is then compared to the current element of $S_2$
		\item if the compared elements are identical, the rest of $S_1$ is appended to $S_a$, including the current element, and then the rest of $S_2$ is appended to $S_a$, excluding the current element.
		\item if either element is void, meaning that there are no more elements in that sequence, the remainder of the non-empty sequence is appended to $S_a$.
	\end{itemize}

	After aggregation $S_a$ possibly contains duplicate executions, which is not possible in a global history.
	Since the rules implied by a sequence only designate what has happened before something else, each subsequent duplicate of an element can be deleted from $S_a$.                      
	This results in a complete order of executions, for example in the graph shown in \ref{fig:global-history-collection-example} where the result would be:
	\begin{itemize}
		\item 
	\end{itemize}
	...
	% From the list of rules, a graph can now be constructed of possible executions, as seen in Figure \ref{fig:execution-sequence-graph-example1}.
	% In order to extract an actual valid execution sequence, each perceived order is aggregated 

	% a topological ordering needs to be applied, but as this is only valid in acyclic graphs, cycles are collapsed into single nodes, as shown in Figure \ref{fig:execution-sequence-graph-example2}.
	% This is still valid, as cycles in the happened-before relationships of executions, entails that the executions happened concurrently.
	% And since the locking rules from section \ref{subsec:event-locking}, only allow concurrent executions when the events are dynamically independent, any linearisation of the concurrent executions must result in the same state of the graph. 
	% After collapsing any cycles, topological ordering can then be performed resulting in a valid execution sequence, for example $A_1D_1B_1E_1E_2F_1F_2C_1$
	...
	% All in all, the complexity of this operation is dominated by the process of ensuring a consistent cut, resulting in the complexity $O(n^2 \cdot m^2)$,which is expensive, but well within the reasonable limit of local computations.
		...
		\subsubsection{Reduction of DCR linearisation to NP}
