...
% Description of components covered in this section and why they are needed.
	...
	The following is a security analysis of the algorithm described in section \ref{sec:analysis}.
		...
		However, under a Dolev-Yao adversary, the adversary can prevent any other process in wining the election, thus prolonging the liveness attack indefinitely.

		Notice that these attacks both happens on a single cluster, and therefore at most affect a subset of the clusters that make up the entire system.
		If the adversary wants to mount an attack on the entire system that prevents liveness in any state of the graph, they must mount a liveness attack on a subset of clusters $A$ for which $\forall e \in A. \not \exists e'. e' \in I_{stat}(e) \land e' \not \in A$.
		...
		An alternative is to attack $I_{dyn}(G,e)$ for the current $G$ and all $e$'s, but the adversary is not guaranteed that the global state reported by the log-augmented snapshot described in~\ref{subsec:global-history-collection}, and is therefore guaranteed that their attack will completely prevent any further execution of graph.
		So an adversary must mount a suboptimal attack to prevent liveness in the entire system.

		Safety cannot be attacked by the safety guarantees provided by Raft.
