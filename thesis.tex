...
\lstdefinestyle{pseudo}{
  mathescape=true,
  escapeinside={<|}{|>},
  morekeywords={function, send, to, receive, from, broadcast, timeout, match, with, when, and, or, if, else, while, do},
  morecomment=[l]{//},
  keepspaces=true,
...
\section{Introduction}
	...
	As DCR graphs are concerned with maintaining a consistent state, the issue of making DCR graphs distributed is closely related to that of maintaining distributed state replications.
	However, the innate properties of DCR graphs would be constrained by existing solutions to distributed state replication, due to the large number of messages required and the low degree of concurrency provided by the strict synchronization of these algorithms.
	...
	In a network of untrusting peers, trust could therefore be guaranteed by Intel, which seems like a very strong base to build a consensus algorithm on.
	...
	\begin{itemize}
		...
		\item A description of a generalized version of that design and implementation supporting arbitrary smart contracts.
		\item An algorithm for achieving consensus in a decentralized and distributed state replication system, using less than $O(n)$ messages, where $n$ is the number of nodes in the network, part of which is an SGX version of Raft~\cite{ongaro_search_2014}.
		\item An argument for Intel SGX's ability to transform any crash stop tolerant algorithm to a byzantine failure tolerant one.
	\end{itemize}

	\subsection{Problem}

	The goal is to create a DCR engine with byzantine fault tolerance, that allows mutually untrusting actors to interact with the events in the graph concurrently.
	...
	The trivial solution to fault tolerance, is to make all peers responsible for the entire state of the system, but given that changes in the graph would then have to be synchronized with all peers, this would violate the goal of a low number of messages.
	...
	\begin{itemize}
		\item Support the replication of a decentralized DCR graph, while maintaining a high degree of concurrency as permitted by the specific DCR graph, as shown in~\cite{debois_concurrency_2015}.
		\item Achieve consensus on a state of a decentralized DCR graph in a number of messages by a constant factor of less than the number of nodes in the graph.
...
\end{description}
	...
	With a better understanding of the prerequisite information, a more thorough definition of the problem can now be presented.
	We define our system as having $n$ peers $\{p_1, p_2 \dots p_n\}$, and a DCR graph $G$, which consists of $v$ events $E = \{e_1, e_2, \dots, e_v\}$.
	Each peer is responsible for one or more events, where the peers responsible for $e$ stores the current and previous states of $e$.
	...
	\begin{itemize}
	 	\item The state of an event is agreed upon in less that $O(n)$ messages,
	 	\item while maintaining fault tolerance
	 	\item and enabling dynamic independence concurrency of event executions.
	 \end{itemize}

	 Additionally, any peer should be able to collect the run, $R_G$, which is compatible with all prior collected runs, meaning that a run collected at logical time, $t$, must be prefixed by the run collected at time $t-1$, or an equivalent run if concurrent executions are present.
	 We assume that collecting a run will always be possible, as long as maintaining the state of the graph is and therefore disregard it at this point in the analysis.
	...
	This ensures a lesser message complexity than SMR approach, given $n > m \cdot (|E_{eff}(G,e)|+1)$.
	Notice that $\forall e. n = m \cdot v \geq m \cdot (|E_{eff}(G,e)|+1)$).
	...
	For these reasons, we present a simple method of collecting the global history of a DCR graph in Section~\ref{subsec:global-history-collection}.
	...
	\end{description}
	Additionally, neighbourhoods can be defined as closed, specifying that the input is included in the output.
	This is denoted with an $C$ prefix: $CN_{in}$ or $CN_{out}$.
	...
	As ordering must necessarily be detrimental to the concurrency of a DCR graph and must also require some amount of communication between events, minimizing the ordering required is a priority.
	Recall the definitions from Section~\ref{subsubsec:concurrency}, where the dependencies in DCR graphs are explained, and realize that dynamic independence describes exactly the cases where ordering is not needed.
	...
	There is however a small caveat to this, as what is possible due to the semantics of DCR graphs grants an advantage which can be leveraged with regards to minimizing ordering.
	...
	This means that events with effects targeting events within $N_{out}(v)$, where $v$ is the event being executed, need to ensure ordering on the targeted events, of $v$'s execution.
	...
	\begin{figure}[ht!]
		...
		\caption{Graph illustrating that ordering only within $CN_{out}(v)$ is insufficient.}
	...
	According to the semantics of DCR relations (see Section~\ref{subsec:dcr}), the enabledness of an event, $v$, can only be changed by state changes in $CN_{in}(v)$, and the state of the event can only be affected by an execution in $CN_{in}(v)$.
	Consider an event $v$.
	Executing $v$ can then change the state and enabledness of any event in $CN_{out}(v)$.
	...
	\end{description}
	Both of these cases can be covered by ordering in $CN_{out}(N_{out}(v))$ or $CN_{in}(v) \cup N_{out}(v)$.

	\begin{figure}[ht!]
		\center
		\includegraphics[scale=0.5]{figures/dcr-graphs/second-degree-no-effect.pdf}
		\caption{Graph illustrating the advantage of locking in $CN_{out}(CN_{out}(v))$ rather than $CN_{in}(v) \cup N_{out}(v)$.}
	...
	An argument can be made here for the $CN_{out}(CN_{out}(v))$ ordering scheme over $CN_{in}(v) \cup N_{out}(v)$, as it allows for optimizations in that ordering is not needed on \texttt{B}, meaning that the former ordering scheme would be more efficient than the latter.
	...
	\begin{itemize}
		\item Consider a graph $\ev{A} \erel \ev{B}, \ev{C} \erel \ev{B}$. When \texttt{A} is executing, it ensures ordering on \texttt{B}. If \texttt{C} then attempts to execute it will need ensure ordering on \texttt{B} as well. As the execution of \texttt{A} and \texttt{C} have exactly the same effects on \texttt{B}, the ordering of these two executions is inconsequential, as they are dynamically independent.
		\item Consider a graph $\ev{A} \irel \ev{B}, \ev{C} \erel \ev{B}$. When \texttt{A} is executing, it ensures ordering on \texttt{B}. If \texttt{C} attempts to execute, it is clear that it must ensure ordering on \texttt{B} as well, however, \texttt{B} should be allowed to execute at any time, as it is dynamically independent with \texttt{A}.
		\item Consider a graph $\ev{A} \irel \ev[100]{B}, \ev[100]{B} \crel \ev{C}$. When \texttt{A} is executing, it will ensure ordering on both \texttt{B} and \texttt{C} due to the constraint originating from \texttt{B} and targeting \texttt{C}. This is not needed, as the condition relation is invalidated by the fact that \texttt{B} is executed.
	\end{itemize}

	Now that the need for ordering has been thoroughly uncovered, the actual method for ensuring ordering must be chosen.
	Ordering can be assured in a number of ways, but following from the goals specified for this solution, there are some that are a better fit than others.
	Consider an optimistic ordering system where events requiring ordering are notified of the execution and provided that no incompatible executions are performed simultaneously, that execution is considered committed.
	If incompatible executions are performed concurrently, however, the execution is discarded.
	This means that there is a period of uncertainty with regards to a certain execution, but as a goal of this solution is the ability of any peer to obtain a run of a graph, which will be compatible with any future run collected, this optimistic method of ordering executions seems less than ideal.
	...
	We define a global history to be a DCR run, i.e. it is a sequence of event executions valid at the time of execution.
	...
	This would require incorporation of the snapshot algorithm into the consensus algorithm applied it each individual cluster, possibly significantly increasing its complexity.
	...
	Given that these events will always present a local sequence adhering to HB1, there must be some point for each process which is consistent with a point in each other process.
	Each event will always be able to present an empty sequence, as an example of a consistent cut.
	Additionally, there will always be a guaranteed consistent cut, defined by the first\footnote{In the logical, happened-before, sense of first.} event cluster to reply with its observed execution sequence, as it will decide the lowest common denominator.
	...
	\begin{figure}[ht!]
		...
		\caption{Example graph illustrating global history collection. The sets within a DCR event represent that event's execution sequence.}
...
\begin{minipage}{\linewidth}
	\begin{lstlisting}[style=pseudo, caption={Reversed postorder DFS traversal algorithm. Adapted from~\cite{_topological_2018}}, label={lst:dfs-traversal}]
...
\end{minipage}
	...
	Raft guarantees the following properties when the amount of faults is $\le \lfloor \frac{n - 1}{2} \rfloor$ (the highest fault tolerance achievable for the consensus problem~\cite{lamport_lower_2006} with the failure model of this project).
	\begin{figure}[ht!]
	  \centering
	...
	We use Raft to implement event locking (Section~\ref{subsec:execution-ordering}) and cluster consensus (Section~\ref{subsec:cluster-consensus}), by exploiting the guarantees given by Figure~\ref{fig:raft-properties}.
	...
	The following is a security analysis of the algorithm described in Section~\ref{sec:analysis}.
		...
		We allow for a strong adversary known as a Dolev-Yao adversary~\cite{dolev_security_1983}, which is an adversary that can coordinate faulty nodes, delay communication and delay correct nodes, but is unable to subvert the cryptographic techniques applied in this solution, and does not have access to our cryptographic secrets.
		Essentially, it is assumed that an adversary has the same limitations in attacking the system, as a byzantine fault has in breaking it.
		This is a quite strong assumption, in that Intel could theoretically be an adversary and would in that case know the processor specific PID key.

		\subsection{Analysis}
		
		That is, if the adversary takes over a process, they cannot change the SGX component without correct processes rejecting any messages the process will send them.
		Thereby they only have the possibility of tampering with the wrapper-components, and the channels, as seen in Section~\ref{sec:transforming-byzantine-faults}.
		But as the wrapper-components only serve to pass messages, this is the same as tampering with the channels.
		As such, an adversary can at most attack by crashing processes, message omissions, delaying messages and tampering with messages, where tampering will be noticed by the correct peers, as the adversary does not have the MAC secret.
		With these restraints, an adversary can induce several attacks on liveness.

		With message omissions and message delays, the adversary is able to give any process a cluster leadership.
		They can do this by intercepting and removing all messages from the current leader, and intercept and remove all messages from candidates timing out before their desired candidate process.

		By chaining these leader elections without resolving any of them, they can mount an attack on the liveness of a cluster.
		One such chain looks like this: omit messages from the leader to $f+1$ processes until one of them, $p_1$, becomes a candidate.
		Then omit messages from $p_1$ to $f+1$ other processes, until another process, $p_2$, becomes a candidate and repeat.
		Thereby the cluster will stay in a continuous election process, and cannot commit any client requests.
		Assuming that the adversary cannot continue omitting messages indefinitely, however, the cluster will eventually successfully complete an election, and resume operation.
		And due to the election pre-election phase, the original leader will still be the leader after the attack has been terminated.
		...
		The proof of FLP-impossibility~\cite{fischer_impossibility_1985} shows that there will always exist a sequence of message omissions that violates availability (i.e. liveness in SMR), if the system is an asynchronous setting and handles crashing processes.
		Therefore, any fault-tolerant SMR in an asynchronous setting will be vulnerable to such an attack.
		This sequence in Raft is present under the leader election; any unresolved heartbeat will eventually result in a leader election, which might then never terminate if no process wins the election, which is what the adversary exploits in this attack.
		...
		Calculating $I_{stat}(e)$ of a all $e's$ is infeasible for large enough graphs (see Section~\ref{subsubsec:concurrency}), so the adversary must either attack an approximation, or all clusters, both of which is inefficient solutions.
		An alternative is to attack the clusters of all dynamically independent event for the current $G$ state, but the adversary is not guaranteed that the global history reported by the log-augmented snapshot described in~\ref{subsec:global-history-collection}, and is therefore guaranteed that their attack will completely prevent any further execution of graph.
		So an adversary must mount a suboptimal attack to prevent liveness in the entire system.

		Safety cannot be attacked under the limitations of our adversary by the safety guarantees provided by Raft~\cite{ongaro_search_2014}.
...
function verify_term($m, p_s$)
  ...
  | $\MSG{\REQ\APPEND, t, \_, \_, \_, \_}$ when $t < term$
    send $\MSG{\RSP\APPEND, term, -1, -1, false}$ to $p_s$
    return false
...
function leader_accept($m, p_s$)
  ...
  // client commands
  | $\MSG{\REQ\COMMAND, tag, \_, \_}$ when $\exists l \in log, l_{(tag)} = tag \land l_{(index)} \le commit_i$ $\longrightarrow$
...
function follower_accept($m, p_s$)
  ...
  | $\MSG{\REQ\APPEND, \_, prev_t, prev_i, c, entries}$ $\longrightarrow$
    ...
    if $c > commit_i$
      $commit_i \leftarrow$ min($c, |log|$)
    send $\MSG{\RSP\APPEND, term, prev_t, prev_i, true, index}$ to $p_s$
    follower_recv()

  // another follower polls before becoming candidate
  // but we already voted in this term
  // or follower has out-of-date log
  // or we have communicated with the leader within min timeout
  | $\MSG{\REQ\POLL, \_, \_, \_}$ when $endorsement \ne nil$
  | $\MSG{\REQ\POLL, \_, last_t, last_i}$ when $\exists l \in log, l_{(term)} > last_t \lor l_{(term)} = last_t \land l_{(index)} > last_i$
  ...
  | $\MSG{\REQ\ELECTION, \_, last_t, last_i}$ when $\exists l \in log, l_{(term)} > last_t \lor l_{(term)} = last_t \land l_{(index)} > last_i$
    send $\MSG{\RSP\ELECTION, term, false}$ to $p_s$
    follower_recv()
