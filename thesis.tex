...
\end{lstlisting}
	...
	The cases where the execution of an event, \texttt{A}, would affect the enabledness of another event, \texttt{B}, seems to encompass all situations where \texttt{B} would need to track the execution of \texttt{A}, as enabledness specifically concerns the effects an execution has on another events ability to execute. %seems to contradict the sentance before
...
%mangler beskrivelse af happend-before og consistent cut
	One way to achieve a global history, is to implement Chandra and Lamport's Snapshot algorithm~\cite{chandy_distributed_1985}.
	This would require incorporation of this algorithm into the snapshot, increasing the complexity even more.
...
%mangler kort beskrivelse af vi bare spørger alle leaders
	...
	To solve this, any execution not present in the sequence returned by the event itself is considered invalid and therefore that execution is deleted from the sequence of all events, as well as all following events in that sequence.
	...
	Each event will always be able to present an empty sequence, as an example of a consistent cut, and since all messages received within the cut are discarded along with all later executions if the message is not sent within the same cut, it follows that it is always possible to obtain a consistent cut. %WHAT?
	...
	After collapsing any cycles, topological ordering can then be performed resulting in the valid execution sequence of $A_1D_1B_1E_1E_2F_1F_2C_1$
