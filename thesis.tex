...
\end{description}
	...
	An argument could be made that reducing $n$ in the SMR approach can achieve the exact same message complexity.
...
\end{snippet}
	...
	% - Sgx sikkerhed (ret til)
    \subsection{Security of SGX}
	All aforementioned hardware guarantees are given by Intel, but since very little of the proprietary technology is documented, or documented contradictory in different documentations as noted in~\cite{costan_intel_2016}, all security assumptions of SGX rely on trust in Intel's hardware implementation.
	...
	This is ensured by preventing untrusted software access to the PRM and enclave's access to other regions of the PRM besides their exclusive region.
	However, while no vulnerabilities have yet been found, which breach the integrity of PRM,~\cite{costan_intel_2016} shows several issues regarding confidentiality.
	Because enclaves use the system page table even for data residing on PRM, it is possible for untrusted software to learn the page access order by manipulating the OS controlled page table. %MORE!
    ...
    % Slå cache attacks sammen
    % meltdown and spectre
    % physical access not needed, due to symetric secret.
    % Encrypted memory - decrypted on read into cache
    % Svært at skrive sikker kode, pointers to unsafe mem.

%NONONONONO	And since our system has very few confidentiality requirements, the known vulnerabilities do not seem problematic for the purposes of this project.
	...
	% - Implementationsbesvære m. SGX
    \subsection{SGX Curiosities}
	Implementing enclaves is cumbersome work, as SGX is very limited in functionality, as described in Section~\ref{subsec:intel-sgx}.
	When inserting data into an enclave, for instance, that data is not allowed to contain any pointers, as those would then point to memory outside of the enclave.
	This means that even programmatically trivial parts of the implementation of an enclave carry with it considerable work.
	In the case of not allowing nested pointers, flattening the structure is one potential solution, meaning a multi-dimensional array or vector would become a one dimensional collection with a separate data structure containing the indices for each original collection.

	% - Størrelsen af TCB i forhold til systemet
	As the performance is severely limited, due to being constrained to a single active thread, minimizing the TCB is essential for a high performance algorithm using SGX. REFREF
	The transformation of crash-fault tolerant algorithms to byzantine fault tolerant algorithms described in this report is specifically described so as to handle any crash-fault tolerant algorithm and makes no attempt to minimize the TCB, but inserts the algorithm into SGX.
	This is in practice extremely taxing on performance and special consideration should be given to each individual algorithm, in order to evaluate where SGX is needed.
	Methods such as the secret sharing scheme used in FastBFT, as described in~\cite{liu_scalable_2016}, could possible be utilised to great effect in our solution.

	% - Scalability v. fault tolerance
    \subsection{Scalability and fault tolerance}
	The balance between scalability and fault tolerance is highly relevant in the designed solution as the fault tolerance in a crash-fault tolerant system is directly defined by how many times data is replicated in the system.
	As we uncovered in this thesis, replicating data has significant costs associated with it, in that replicas need to be updated, making it costly to perform updates in the system.
	If the scalability of the system is deemed insignificant, or fault tolerance is to be prioritized, making all peers responsible for the entirety of the state accomplishes this.
	This does however make the solution identical to a SMR solution.

	% - PSMR
    \subsection{Partial state machine replication}
	The designed solution solves the issue of partial state machine replication, a problem which has few efficient solutions and none of them accomplishing it using SGX or other TEE's, as far as the authors of this thesis are aware.

	% - Sammenlign med Eth løsning (evt. FastBFT)
	...
	Ethereum has an average block frequency of 15 seconds at this time, performing a series of executions takes a considerable amount of time, compared to the SGX DCR engine.
